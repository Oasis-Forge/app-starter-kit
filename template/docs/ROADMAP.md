...
- [ ] The release gate: the CI version check, then a build-as-a-check on merge. A release is the user's call, not every merge; the check passes a branch that leaves the version alone. Nothing is published from CI and nothing is tagged; the store artifact is built locally and uploaded by hand (`docs/RELEASING.md`). `0.1.0` is the first.
