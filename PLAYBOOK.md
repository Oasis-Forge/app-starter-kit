...
5. `/verify`.
6. `/ship`: coverage of the changed files, missing tests, docs, a release if the user asks for one, driving the built app by hand, then the PR (`--body-file`).
...
- **A release is the user's call, and the gate does not force one.** `scripts/version.sh check` reads the version on `origin/main` and passes a PR that leaves it exactly there — most branches do, and their entries wait under **Unreleased**. A PR that moves the version is checked in full: above the trunk's, with its changelog entry. On main itself there is no PR to gate: the merge build asks `scripts/version.sh released` whether the merge raised the version above the commit before it (it needs `fetch-depth: 2`), builds and checks either way, and says in its summary when a merge was not a release. That is the ordinary case, and it is also what a dependency bump and a PR merged behind another look like; none of them is a failure, because nothing on main is fixed by a red build, and a gate that went red on every dependency bump was one nobody would read. Merge with a merge commit or a squash: after "Rebase and merge" the commit before main's tip can be the PR's own, so a release whose bump was not its last commit reads as none.
